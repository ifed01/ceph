...
void Client::insert_readdir_results(MetaRequest *request, MetaSession *session,
  ...
  if (!p.end()) {
    ...
    bool snapdiff_results = ((unsigned)flags & CEPH_READDIR_SNAPDIFF);
	...
	dn = link(effective_dir, dname, in, NULL);
      ...
      if (hash_order) {
	unsigned hash;
	if (snapdiff_results && dname[0] == SNAPDIFF_RM_INDICATOR) {
	  hash = ceph_frag_value(effective_diri->hash_dentry_name(dname.substr(1)));
	  ldout(cct, 5) << __func__ << "  hash 0x" << hex << hash << dec << ": '" << dname << " -> " << dname.substr(1) << dendl;
	} else {
	  hash = ceph_frag_value(effective_diri->hash_dentry_name(dname));
	}
	if (hash != last_hash)
	  readdir_offset = 2;
	last_hash = hash;
