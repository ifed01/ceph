                           ...
                           vector<CDentry*> *pdnvec, CInode **pin)
{
  ...
  for (unsigned depth = 0; depth < path.depth(); ) {
	...
	return -CEPHFS_EINVAL;
      ...
      auto [snapid_new, is_diff, snapid_diff_other] = realm->resolve_snapname(path[depth], cur->ino());
      if (is_diff && snapid_new < snapid_diff_other) {
	std::swap(snapid_new, snapid_diff_other);
      }
