...
void Server::handle_client_readdir_snapdiff(MDRequestRef& mdr)
{
  ...
  mdr->snapid_diff_other = (uint64_t)req->head.args.snapdiff.snapA;
  mdr->snapid = (uint64_t)req->head.args.snapdiff.snapB;
  //mdr->is_snapid_diff = true;

  dout(10) << __func__
    << " snapid " << mdr->snapid
    << "vs. snapid_diff_other " << mdr->snapid_diff_other
    << dendl;
  ...
  std::function<bool (int, CDentry*, CInode*, bool)> add_result_cb)
{
  ...
  while(it != dir->end()) {
	...
	continue;
      ...
      string_view name_before = before.dn ? before.dn->get_name() : string();
