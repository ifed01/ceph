...
{
  if (--nref == 0) {
    ldout(get_collection()->store->cct, 20) << __func__ << " " << this
...
void BlueStore::Blob::split(Collection *coll, uint32_t blob_offset, Blob *r)
{
  ...
  ceph_assert(!r->is_shared()); // FIXME: is this valid
  ...
  if (is_shared()) {
    shared_blob()->bc.split(shared_blob()->get_cache(), blob_offset, r->shared_buffer_space()->bc);
  } else {
    //FIXME: do we really need split, is r always non-shared?
    ceph_assert(!r->is_shared());
    shared_buffer_space()->bc.split(
      shared_buffer_space()->get_cache(),
      blob_offset,
      r->shared_buffer_space()->bc);
  }
    ...
    if (!p->second->oid.match(destbits, destpg.pgid.ps())) {
      ...
      for (auto& e : o->extent_map.extent_map) {
        if (e.blob->is_shared()) {
          ...
          bcvec.push_back(e.blob->shared_buffer_space().get());
        }
      }
      for (auto& b : o->extent_map.spanning_blob_map) {
        if (b.second->is_shared()) {
          ...
          bcvec.push_back(b.second->shared_buffer_space().get());
        }
      }
      for (auto sb : sbvec) {
  ...
  blobs2read_t& blobs2read)
{
  ...
  while (left > 0 && lp != o->extent_map.extent_map.end()) {
    ...
    bptr->buffer_space()->bc.read(
      bptr->buffer_space()->get_cache(), b_off, b_len, cache_res, cache_interval,
      read_cache_policy);
  ...
  while (b2r_it != blobs2read.end()) {
    ...
    if (bptr->get_blob().is_compressed()) {
      ...
      if (r < 0)
        return r;
      if (buffered) {
        bptr->buffer_space()->bc.did_read(bptr->buffer_space()->get_cache(), 0,
                                       raw_bl);
      }
      for (auto& req : r2r) {
        for (auto& r : req.regs) {
      ...
      for (auto& req : r2r) {
        ...
        if (buffered) {
          bptr->buffer_space()->bc.did_read(bptr->buffer_space()->get_cache(),
                                         req.r_off, req.bl);
        }
