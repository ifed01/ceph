  ...
  WriteContext *wctx)
{
  ...
  for (auto& wi : wctx->writes) {
	...
	txc->statfs_ex_delta.original_compressed += l->length();
	l = &compressed_bl;
	final_length = newlen;
	csum_length = newlen;
  ...
  WriteContext *wctx)
{
  ...
  for (auto &l : wctx->lex_old) {
    ...
    if (b->ref_map.empty()) {
      if (b->is_compressed()) {
	txc->statfs_ex_delta.stored_compressed -= b->get_payload_length();
	txc->statfs_ex_delta.original_compressed -= b->get_length;
      }
