  ...
  TrackedOpRef osd_op)
{
  ...
  c->osr->queue_new(txc);
  dout(20) << __func__ << " osr " << c->osr.get() << " = " << txc
           << dendl;
  return txc;
}
...
void BlueStore::_txc_state_proc(TransContext *txc)
{
  ...
  while (true) {
    ...
    case TransContext::STATE_IO_DONE:
      ceph_assert(ceph_mutex_is_locked(txc->get_osr()->qlock));  // see _txc_finish_io
      if (txc->had_ios) {
	++txc->get_osr()->txc_with_unstable_io;
      }
	...
	if (txc->last_nid >= nid_max ||
	  ...
	  dout(20) << __func__
		   ...
		   << dendl;
	} else if (txc->get_osr()->kv_committing_serially) {
	...
	} else if (txc->get_osr()->txc_with_unstable_io) {
	  dout(20) << __func__ << " prior txc(s) with unstable ios "
		   << txc->get_osr()->txc_with_unstable_io.load() << dendl;
	...
	if (txc->get_state() != TransContext::STATE_KV_SUBMITTED) {
	  ...
	  ++txc->get_osr()->kv_committing_serially;
	}
	if (txc->had_ios)
	  kv_ios++;
...
void BlueStore::_txc_apply_kv(TransContext *txc, bool sync_submit_transaction)
{
  ceph_assert(txc->get_state() == TransContext::STATE_KV_QUEUED);
  {
    ...
    if (txc->get_osr()->kv_submitted_waiters) {
      std::lock_guard l(txc->get_osr()->qlock);
      txc->get_osr()->qcond.notify_all();
    }
...
{
  dout(20) << __func__ << " txc " << txc << " osr " << txc->get_osr() << dendl;

  DeferredBatch *tmp;
  txc->get_osr()->deferred_lock.lock();
  {
    if (!txc->get_osr()->deferred_pending) {
      tmp = new DeferredBatch(cct, txc->get_osr());
    } else {
      tmp  = txc->get_osr()->deferred_pending;
    }
  }
  ...
  {
    ++deferred_queue_size;
    txc->get_osr()->deferred_pending = tmp;
    // condition "tmp->txcs.size() == 1" mean deferred_pending was originally empty.
    // So we should add osr into deferred_queue.
    if (!txc->get_osr()->deferred_running && (tmp->txcs.size() == 1)) {
      deferred_lock.lock();
      deferred_queue->push_back(*txc->get_osr());
      deferred_lock.unlock();
    }

    if (deferred_aggressive &&
	!txc->get_osr()->deferred_running) {
      _deferred_submit_unlock(txc->get_osr());
    } else {
      txc->get_osr()->deferred_lock.unlock();
    }
  }
 }
...
void BlueStore::deferred_try_submit()
{
...

  {
    ...
    for (auto& osr : *deferred_queue) {
      osrs.push_back(OpSequencerRef(& osr));
    }
  }
...
int BlueStore::_deferred_replay()
{
  ...
  for (it->lower_bound(string()); it->valid(); it->next(), ++count) {
    ...
    if (has_some) {
      TransContext *txc = _txc_create(ch.get(), nullptr);
 ...
 out:
  ...
  _osr_register_zombie(ch->osr.get());
  _osr_drain_all();
  if (fake_ch) {
    new_coll_map.clear();
  }
  ...
  ThreadPool::TPHandle *handle)
{
  ...
  // prepare
  TransContext *txc = _txc_create(static_cast<Collection*>(ch.get()),
				  &on_commit, op);
...
void BlueStore::debug_punch_hole(
  ...
  uint32_t len)
{
  BlueStore::TransContext txc(cct, c.get(), nullptr);
...
{
  o->extent_map.punch_hole_2(offset, length, released,
...
void BlueStore::_shutdown_cache()
{
  ...
  for (auto& p : coll_map) {
    ...
    if (!p.second->shared_blob_set.empty()) {
      ...
      dump_shared_blob_set<0>(cct, p.second->shared_blob_set);
    }
