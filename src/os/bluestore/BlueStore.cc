  ...
  uint64_t blob_len)
{
  ...
  for (auto w : writes) {
    if (b == w.b) {
      auto loffs2 = P2ALIGN(w.logical_offset, w.blob_length);
      if ((loffs <= loffs2 && (loffs + blob_len) > loffs2) ||
          (loffs >= loffs2 &&  loffs < (loffs2 + w.blob_length))) {
        return true;
      }
    }
  }
  ...
  while (ep != o->extent_map.extent_map.end()) {
    ...
    if (!head_read && !tail_read &&
      ...
      bool conflicting = llen == b->get_blob().get_logical_length() &&
                           wctx->has_conflict(b, offset, alloc_len);
      if (!conflicting) {
