...
bool BluestoreWAL::get_write_pos(size_t need, uint64_t* _need_pages)
{
  ...
  ceph_assert(need_pages < get_max_pages());
  if (avail / page_size < need_pages) {
    return false;
  }
  // pending wiping pages can't be used and prevent 
  // following outdated pages from usage as well
  // hence measuring how many outdated pages are available
  if (last_wiped_page_seqno < last_wiping_page_seqno) {
    ceph_assert(get_max_pages() >= page_seqno - last_wiped_page_seqno);
    uint64_t has_pages = get_max_pages() - (page_seqno - last_wiped_page_seqno);
    if (has_pages < need_pages) {
      return false;
    }
  }
  // sanity check
  uint64_t avail_total_pages = get_max_pages() - (page_seqno - last_outdated_page_seqno);
  ceph_assert(need_pages <= avail_total_pages);
  ceph_assert(avail_total_pages == avail / page_size);
  ...
  uint64_t avail_pages = get_max_pages() - (page_seqno - last_wiping_page_seqno);
...
size_t BluestoreWAL::wipe_pages(IOContext* ioc)
{
  ...
  dout(7) << __func__ << " " << last_wiping_page_seqno << "->"
	  << last_outdated_page_seqno
	  << dendl;
  ceph_assert(last_wiping_page_seqno < last_outdated_page_seqno);
  wiping = last_outdated_page_seqno - last_wiping_page_seqno;
  ceph_assert(wiping < get_max_pages());
  ...
  } while (last_wiping_page_seqno < last_outdated_page_seqno);
  free(buf);
  return wiping;
}
...
void BluestoreWAL::aio_finish(BlueStore* store, Op& op)
{
  ...
  if (op.transact_seqno == min_pending_seqno) {
    ...
    pending_transactions.erase_and_dispose(
      ...
      DeleteDisposer());
    ++min_pending_seqno;
    while (min_pending_seqno <= max_pending_seqno) {
      auto it = pending_transactions.find(Op(min_pending_seqno));
...
int BluestoreWAL::submit(void* txc, bufferlist& t)
{
  ...
  while (!get_write_pos(t_len, &need_pages)) {
    dout(7) << __func__ << " - no write pos, waiting" << dendl;
    flush_cond.wait(l);
  ...
  size_t wiping = 0;
  if (last_wiping_page_seqno < last_outdated_page_seqno) {
  ...
  max_pending_seqno = transact_seqno;
...
void BluestoreWAL::committed(uint64_t outdated_page_seqno)
{
  if (outdated_page_seqno) {
    // When submitting multi-page ops we might get second commit 
    // indications for the last page seqno.
    // Just ignore non-first one.
    if (outdated_page_seqno == last_outdated_page_seqno) {
      return; 
    }

    std::unique_lock l(lock);

    ceph_assert(outdated_page_seqno > last_outdated_page_seqno);
    auto num_pages = outdated_page_seqno - last_outdated_page_seqno;
    avail += num_pages * page_size;
    last_outdated_page_seqno = outdated_page_seqno;

    //awake pending submits only if there is no 
    // page wiping in progress as it prevents page from the reuse 
    // until the completion. 
    if (last_wiping_page_seqno == last_wiped_page_seqno) {
      flush_cond.notify_all();
    }
  }
}
