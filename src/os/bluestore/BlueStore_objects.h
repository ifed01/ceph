...
namespace bluestore {
  ...
  struct SharedBlobSet {
    ...
    SharedBlob* lookup(uint64_t sbid);
  ...
  //defining through a struct to be able to have forward declaration
  struct blob_map_t : public mempool::bluestore_cache_meta::map<int, BlobRef>
  {
  };
  ...
  struct ExtentMap {
    ...
    void dup(BlueStore* b, BlueStore::TransContext*, CollectionRef&, OnodeRef&, OnodeRef&,
      uint64_t&, uint64_t&, uint64_t&);
    void dup_esb(BlueStore* b, BlueStore::TransContext*, CollectionRef&, OnodeRef&, OnodeRef&,
      uint64_t&, uint64_t&, uint64_t&);
  ...
  */
  ...
  struct TransContext final : public AioContext {
    ...
    explicit TransContext(CephContext* cct, BlueStore::Collection* c,
...
template <int LogLevelV>
void dump_shared_blob_set(CephContext* cct, bluestore::SharedBlobSet& sbs)
{
  std::lock_guard l(sbs.lock);
  for (auto& i : sbs.sb_map) {
