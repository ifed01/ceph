  ...
  const vector<string>& devs)
{
  for (auto& i : devs) {
    ...
    int r = -1; //FIXME BlueStore::_read_bdev_label(cct, i, &label);
    if (r < 0) {
  ...
  bool* has_wal)
{
  ...
  for (auto& d : devs) {
    bluestore_bdev_label_t label;
    int r = -1; //FIXME: BlueStore::_read_bdev_label(cct, d, &label);
    if (r < 0) {
...
int main(int argc, char **argv)
{
  ...
  if (action == "restore_cfb") {
    ...
    bluestore_bdev_label_t label;
    int r = -1; //FIXME BlueStore::_read_bdev_label(cct.get(), devs.front(), &label);
    if (r < 0) {
    ...
    for (auto& i : devs) {
      bluestore_bdev_label_t label;
      int r = -1; //FIXME BlueStore::_read_bdev_label(cct.get(), i, &label);
      if (r < 0) {
	cerr << "unable to read label for " << i << ": "
	     << cpp_strerror(r) << std::endl;
	exit(EXIT_FAILURE);
    ...
    r = -1; //FIXME BlueStore::_write_bdev_label(cct.get(), devs.front(), label);
    if (r < 0) {
