...
static ostream& _prefix(std::ostream *_dout, TransparentBackend *pgb) {
...
struct TransparentBackend::C_OSD_RepModifyCommit : public Context {
  TransparentBackend *pg;
  RepModifyRef rm;
  C_OSD_RepModifyCommit(TransparentBackend *pg, RepModifyRef r)
    : pg(pg), rm(r) {}
...
TransparentBackend::TransparentBackend(
  PGBackend::Listener *pg,
  const coll_t &coll,
...
void TransparentBackend::run_recovery_op(
...
int TransparentBackend::recover_object(
...
void TransparentBackend::check_recovery_sources(const OSDMapRef& osdmap)
{
...
bool TransparentBackend::can_handle_while_inactive(OpRequestRef op)
{
...
bool TransparentBackend::_handle_message(
  OpRequestRef op
  )
{
  dout(10) << __func__ << ": " << *(op->get_req()) << " " << op->get_req()->get_type() << dendl;
  /*switch (op->get_req()->get_type()) {
...
void TransparentBackend::clear_recovery_state()
{
...
void TransparentBackend::on_change()
{
...
int TransparentBackend::objects_read_sync(
  const hobject_t &hoid,
  uint64_t off,
  uint64_t len,
  uint32_t op_flags,
...
void TransparentBackend::objects_read_async(
  ...
  bool fast_read)
{
  ceph_abort_msg("async read is not used by transparent pool");
}
...
int TransparentBackend::be_deep_scrub(
...
/*
void TransparentBackend::_do_push(OpRequestRef op)
{
...
/*struct C_TransparentBackend_OnPullComplete : GenContext<ThreadPool::TPHandle&> {
  TransparentBackend *bc;
  list<TransparentBackend::pull_complete_info> to_continue;
  int priority;
  C_TransparentBackend_OnPullComplete(TransparentBackend *bc, int priority)
    : bc(bc), priority(priority) {}

  void finish(ThreadPool::TPHandle &handle) override {
    TransparentBackend::RPGHandle *h = bc->_open_recovery_op();
...
void TransparentBackend::_do_pull_response(OpRequestRef op)
{
  auto m = op->get_req<MOSDPGPush>();
  ceph_assert(m->get_type() == MSG_OSD_PG_PUSH);
  pg_shard_t from = m->from;
  ...
  if (!to_continue.empty()) {
    C_TransparentBackend_OnPullComplete *c =
      new C_TransparentBackend_OnPullComplete(
	this,
	m->get_priority());
...
void TransparentBackend::do_pull(OpRequestRef op)
{
...
void TransparentBackend::do_push_reply(OpRequestRef op)
{
...
/*Message * TransparentBackend::generate_subop(
  const hobject_t &soid,
  const eversion_t &at_version,
  ceph_tid_t tid,
  osd_reqid_t reqid,
...
void TransparentBackend::issue_op(
  const hobject_t &soid,
  const eversion_t &at_version,
  ceph_tid_t tid,
  osd_reqid_t reqid,
...
// sub op modify
void TransparentBackend::do_repop(OpRequestRef op)
{
...
void TransparentBackend::repop_commit(RepModifyRef rm)
{
...
// ===========================================================
/*
void TransparentBackend::calc_head_subsets(
...
void TransparentBackend::calc_clone_subsets(
...
void TransparentBackend::prepare_pull(
  eversion_t v,
  const hobject_t& soid,
 ...
 */
/*int TransparentBackend::prep_push_to_replica(
...
int TransparentBackend::prep_push(ObjectContextRef obc,
...
int TransparentBackend::prep_push(
  ObjectContextRef obc,
  const hobject_t& soid, pg_shard_t peer,
  eversion_t version,
  interval_set<uint64_t> &data_subset,
...
void TransparentBackend::submit_push_data(
...
void TransparentBackend::submit_push_complete(
  const ObjectRecoveryInfo &recovery_info,
  ObjectStore::Transaction *t)
{
...
ObjectRecoveryInfo TransparentBackend::recalc_subsets(
  const ObjectRecoveryInfo& recovery_info,
  SnapSetContext *ssc,
  ObcLockManager &manager)
{
...
bool TransparentBackend::handle_pull_response(
...
void TransparentBackend::handle_push(
...
void TransparentBackend::send_pushes(int prio, map<pg_shard_t, vector<PushOp> > &pushes)
{
...
void TransparentBackend::send_pulls(int prio, map<pg_shard_t, vector<PullOp> > &pulls)
{
...
int TransparentBackend::build_push_op(const ObjectRecoveryInfo &recovery_info,
...
void TransparentBackend::prep_push_op_blank(const hobject_t& soid, PushOp *op)
{
...
bool TransparentBackend::handle_push_reply(
...
void TransparentBackend::handle_pull(pg_shard_t peer, PullOp &op, PushOp *reply)
{
  const hobject_t &soid = op.soid;
  struct stat st;
 ...
 */
/*void TransparentBackend::trim_pushed_data(
...
void TransparentBackend::_failed_pull(pg_shard_t from, const hobject_t &soid)
{
...
void TransparentBackend::clear_pull_from(
...
void TransparentBackend::clear_pull(
  map<hobject_t, PullInfo>::iterator piter,
...
int TransparentBackend::start_pushes(
  const hobject_t &soid,
  ObjectContextRef obc,
  RPGHandle *h)
{
