...
void TransparentPG::do_request(OpRequestRef& op, ThreadPool::TPHandle& handle)
{
  ...
  if (!have_same_or_newer_map(op->min_epoch)) {
    dout(0) << __func__ << " min " << op->min_epoch
  ...
  if (can_discard_request(op)) {
    dout(0) << __func__ << " discarded " << dendl;
    return;
  }
...
/*  auto it_objects = recovery_state.get_pg_log().get_log().objects.find(soid);
  ...
  if (obc) {
    ...
    dout(0) << __func__ << ": obc NOT found in cache: " << soid
            << " " << ch->cid << dendl;
    // check disk
    bufferlist bv;
    if (attrs) {
