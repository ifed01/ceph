...
uint64_t cls_get_pool_stripe_width(cls_method_context_t hctx)
{
  ...
  return ctx->get_pg()->get_pool().info.stripe_width;
}
...
/*  OpContextBase **pctx = (OpContextBase**)hctx;
  ...
  return (*pctx)->get_pg()->start_cls_gather(*pctx, &gather.src_obj_buffs, pool, cls, method, inbl);*/
  ceph_assert(false);
  return -1; //FIXME
}
