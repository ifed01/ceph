...
{
  OpContextBase **pctx = (OpContextBase **)hctx;
  bufferlist idata;
  vector<OSDOp> nops(1);
  OSDOp& op = nops[0];
  int r;
  ...
  r = (*pctx)->get_pg()->do_osd_ops(*pctx, nops);
  if (r < 0)
    return r;
	     ...
	     char **outdata, int *outdatalen)
{
  ...
  int r = (*pctx)->get_pg()->do_osd_ops(*pctx, ops);
  if (r < 0)
    return r;
...
{
  OpContextBase **pctx = (OpContextBase **)hctx;
  *origin = (*pctx)->get_op()->get_req()->get_orig_source_inst();
  return 0;
}
...
int cls_cxx_create(cls_method_context_t hctx, bool exclusive)
{
  ...
  return (*pctx)->get_pg()->do_osd_ops(*pctx, ops);
}
...
int cls_cxx_stat(cls_method_context_t hctx, uint64_t *size, time_t *mtime)
{
  ...
  ret = (*pctx)->get_pg()->do_osd_ops(*pctx, ops);
  if (ret < 0)
    return ret;
...
{
  OpContextBase *ctx = *(OpContextBase**)hctx;

  return ctx->get_pg()->get_last_user_version();
}
...
{
  OpContextBase *ctx = *(OpContextBase**)hctx;
  return ctx->get_pg()->get_manifest_ref_count(ctx->obc, fp_oid, ctx->get_op());
}

uint64_t cls_get_osd_min_alloc_size(cls_method_context_t hctx) {
  OpContextBase *ctx = *(OpContextBase**)hctx;

  return ctx->get_pg()->get_min_alloc_size();
}
...
uint64_t cls_get_pool_stripe_width(cls_method_context_t hctx)
{
  ...
  return ctx->get_pg()->get_pgpool().info.stripe_width;
}
		   ...
		   const char *cls, const char *method, bufferlist& inbl)
{
  ...
  OpContextBase **pctx = (OpContextBase**)hctx;
  return (*pctx)->get_pg()->start_cls_gather(*pctx, src_objs, pool, cls, method, inbl);
}

int cls_cxx_get_gathered_data(cls_method_context_t hctx, std::map<std::string, bufferlist> *results)
{
  OpContextBase **pctx = (OpContextBase**)hctx;
  return (*pctx)->get_pg()->get_cls_gathered_data(*pctx, results);
}
