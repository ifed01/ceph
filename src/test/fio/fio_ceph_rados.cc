...
//FIXME: make intrusive list item
class UnitComplete {
  io_u* u = nullptr;
    ...
    unlink(td->o.unlink)
{
 ...
 /*
  ...
  const char* pool_name = "fio_pool"; //FIXME: 
  ...
  for (uint32_t i = 0; i < td->o.nr_files; i++) {
    ...
    objects.emplace_back(f->file_name);
    io_ctx.create(f->file_name, false);
  }
  /*cerr<<"objects created:"<<td->o.nr_files<<std::endl;*/
}
