...
public:
    ...
    std::function<bool(const dirent*, uint64_t)> fn)
  {
    ...
    struct ceph_dir_result* ls_dir;
    struct ceph_dir_result* ls_other_dir;
    int r = ceph_opendir(cmount, snap1_path.c_str(), &ls_dir);
    if (r != 0) {
    std::cerr << snap1_path << " failed to open, ret:" << r << std::endl;
      return r;
    }
    r = ceph_opendir(cmount, snap2_path.c_str(), &ls_other_dir);
    if (r != 0) {
    std::cerr << snap2_path << " failed to open, ret:" << r << std::endl;
      return r;
    }
    dirent res_de;
    uint64_t res_snapid;
    ...
    ceph_assert(0 == ceph_closedir(cmount, ls_other_dir));
    ceph_assert(0 == ceph_closedir(cmount, ls_dir));
    return r;
  }
...
{
  //************ snap1 *************
  ASSERT_LT(0, write_full("a", "file 'a' v1"));
  ASSERT_LT(0, write_full("b", "file 'b' v1"));
  ASSERT_LT(0, write_full("c", "file 'c' v1"));
  ASSERT_LT(0, write_full("f", "file 'f' v1"));
  ASSERT_LT(0, write_full("ff", "file 'ff' v1"));
  ASSERT_LT(0, write_full("g", "file 'g' v1"));
  ASSERT_LT(0, write_full("i", "file 'i' v1"));

  ASSERT_EQ(0, mkdir("S"));
  ASSERT_LT(0, write_full("S/sa", "file 'S/sa' v1"));

  ASSERT_EQ(0, mkdir("C"));
  ASSERT_LT(0, write_full("C/cc", "file 'C/cc' v1"));

  ASSERT_EQ(0, mkdir("C/C1"));
  ASSERT_LT(0, write_full("C/C1/c", "file 'C/C1/c' v1"));

  ASSERT_EQ(0, mkdir("G"));
  ASSERT_LT(0, write_full("G/gg", "file 'G/gg' v1"));

  ASSERT_EQ(0, mkdir("I"));
  ASSERT_LT(0, write_full("I/ii", "file 'I/ii' v1"));
  ASSERT_LT(0, write_full("I/iii", "file 'I/iii' v1"));

  ASSERT_EQ(0, mkdir("L"));
  ASSERT_LT(0, write_full("L/ll", "file 'L/ll' v1"));
  ASSERT_EQ(0, mkdir("L/LL"));

  ASSERT_EQ(0, mksnap("snap1"));
  //************ snap2 *************

  ASSERT_LT(0, write_full("b", "file 'b' v2"));
  ASSERT_EQ(0, unlink("c"));
  ASSERT_LT(0, write_full("d", "file 'd' v2"));
  ASSERT_LT(0, write_full("f", "file 'f' v2"));
  ASSERT_EQ(0, unlink("g"));

  ASSERT_LT(0, write_full("S/sa", "file 'S/sa' v2"));

  ASSERT_EQ(0, mkdir("T"));
  ASSERT_LT(0, write_full("T/td", "file 'T/td' v2"));

  ASSERT_EQ(0, purge_dir("C"));
  ASSERT_EQ(0, purge_dir("G"));

  ASSERT_LT(0, write_full("k", "file 'k' v2"));
  ASSERT_LT(0, write_full("l", "file 'l' v2"));

  ASSERT_EQ(0, mkdir("K"));
  ASSERT_LT(0, write_full("K/kk", "file 'K/kk' v2"));

  ASSERT_LT(0, write_full("I/ii", "file 'I/ii' v2"));

  ASSERT_EQ(0, mkdir("I/J"));
  ASSERT_LT(0, write_full("I/J/i", "file 'I/J/i' v2"));
  ASSERT_LT(0, write_full("I/J/j", "file 'I/J/j' v2"));
  ASSERT_LT(0, write_full("I/J/k", "file 'I/J/k' v2"));

  ASSERT_LT(0, write_full("L/LL/ll", "file 'L/LL/ll' v2"));

  ASSERT_EQ(0, mkdir("L/LM"));
  ASSERT_LT(0, write_full("L/LM/lm", "file 'L/LM/lm' v2"));
  ...
  ASSERT_LT(0, write_full("a", "file 'a' v3"));
  ASSERT_LT(0, write_full("b", "file 'b' v3"));
  ASSERT_LT(0, write_full("d", "file 'd' v3"));
  ASSERT_EQ(0, unlink("f"));
  ASSERT_EQ(0, unlink("ff"));
  ASSERT_LT(0, write_full("g", "file 'g' v3"));

  ASSERT_LT(0, write_full("S/sa", "file 'S/sa' v3"));

  ASSERT_LT(0, write_full("T/td", "file 'T/td' v3"));

  ASSERT_EQ(0, mkdir("G"));
  ASSERT_LT(0, write_full("G/gg", "file 'G/gg' v3"));
  ...
  ASSERT_EQ(0, mkdir("H"));
  ASSERT_LT(0, write_full("H/hh", "file 'H/hh' v3"));

  ASSERT_LT(0, write_full("I/ii", "file 'I/ii' v3"));
  ASSERT_LT(0, write_full("I/iii", "file 'I/iii' v3"));
  ASSERT_LT(0, write_full("I/iiii", "file 'I/iiii' v3"));

  ASSERT_LT(0, write_full("I/J/i", "file 'I/J/i' v3"));
  ASSERT_EQ(0, unlink("I/J/k"));
  ASSERT_LT(0, write_full("I/J/l", "file 'I/J/l' v3"));

  ASSERT_LT(0, write_full("L/ll", "file 'L/ll' v3"));

  ASSERT_LT(0, write_full("L/LL/ll", "file 'L/LL/ll' v3"));

  ASSERT_EQ(0, purge_dir("L/LM"));

  ASSERT_EQ(0, mksnap("snap3"));
  //************ head *************
  ASSERT_LT(0, write_full("a", "file 'a' head"));

  ASSERT_LT(0, write_full("h", "file 'h' head"));

  ASSERT_LT(0, write_full("S/sh", "file 'S/sh' head"));

  ASSERT_EQ(0, unlink("l"));

  ASSERT_EQ(0, purge_dir("I"));

  ASSERT_LT(0, write_full("L/LL/ll", "file 'L/LL/ll' head"));
...
TEST(LibCephFS, SnapDiffLib2)
{
  ...
  ASSERT_LT(0, test_mount.write_full("fileA", "hello world"));
  ASSERT_LT(0, test_mount.write_full("fileC", "hello world to be removed temporarily"));
  ASSERT_LT(0, test_mount.write_full("fileD", "hello world unmodified"));
  ASSERT_LT(0, test_mount.write_full("fileF", "hello world to be removed completely"));
  ASSERT_EQ(0, test_mount.mkdir("dirA"));
  ASSERT_LT(0, test_mount.write_full("dirA/fileA", "file 'A/a' v1"));
  ASSERT_EQ(0, test_mount.mkdir("dirC"));
  ASSERT_LT(0, test_mount.write_full("dirC/filec", "file 'C/c' v1"));
  ...
  ASSERT_LT(0, test_mount.write_full("fileB", "hello world in B"));
  ASSERT_EQ(0, test_mount.unlink("fileC"));
  ASSERT_EQ(0, test_mount.unlink("fileF"));
  ...
  auto verify_snap_diff = [&](
    std::string_view snap1_name,
    std::string_view snap2_name,
    vector<pair<string, uint64_t>>& expected)
  {
    string snap_path = test_mount.make_snap_path(snap2_name.data());
    string snap_other_path = test_mount.make_snap_path(snap1_name.data());
    std::cout << "--------- " << snapid1 << " vs. " << snap2_name
              << " diff listing verification---------" << std::endl;
    ASSERT_EQ(0,
      test_mount.readdir_snapdiff_and_compare(snap_path.c_str(),
        snap_other_path.c_str(),
        expected));
  };
  auto print_snap_diff = [&](std::string_view snap1_name, std::string_view snap2_name)
  {
    string snap_path = test_mount.make_snap_path(snap2_name.data());
    string snap_other_path = test_mount.make_snap_path(snap1_name.data());
    std::cout << "---------" << snapid1 << " vs. " << snap2_name
              << " diff listing---------" << std::endl;
    ASSERT_EQ(0, test_mount.for_each_readdir_snapdiff(
      snap_path.c_str(),
      snap_other_path.c_str(),
      [&](const dirent* dire, uint64_t snapid) {
  ...
  verify_snap_diff("snap1", "snap2", snap1_2_diff_expected);
  verify_snap_listing(); // served from cache
  verify_snap_diff("snap2", "snap1", snap1_2_diff_expected);
  verify_snap_listing(); // served from cache
  verify_snap_diff("snap1", "snap3", snap1_3_diff_expected);
  verify_snap_diff("snap2", "snap3", snap2_3_diff_expected);
  verify_snap_listing(); // served from cache
  ...
  verify_snap_listing(); // served from cache
  verify_snap_diff("snap1", "snap2", snap1_2_diff_expected);
  verify_snap_diff("snap3", "snap2", snap2_3_diff_expected);
  verify_snap_diff("snap1", "snap3", snap1_3_diff_expected);
