...
TEST(BlueStoreWAL, basic) {
  ...
  /////////////////////////////////////////////////
  //1 huge transaction to take all the space but the current page
  //
  t.reset_all();
  auto num_pages_w9 = page_count - 2; // = 14
  auto page_seqno_before_w9 = t.get_page_seqno();
  transactions.push_back(
    std::string(num_pages_w9 * (psize - head_size), 'e'));
  ...
  // page seqno to be increased by num_pages_w9 + 1 due to "self-submission"
  // indication
